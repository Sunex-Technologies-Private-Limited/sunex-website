...
function parseArguments(args) {
  return { manifestOnly: args.includes("--manifest-only") };
}

function getRequiredSourceOrigin() {
  const value = process.env.SUNEX_ASSET_SOURCE_URL;
  if (!value) {
    throw new Error("Set SUNEX_ASSET_SOURCE_URL to the published website origin before exporting assets.");
  }
...
const { manifestOnly } = parseArguments(process.argv.slice(2));
...
for (const key of assetKeys) {
  await exportAsset(sourceOrigin, outputDirectory, key);
}

await writeFile(
  path.join(outputDirectory, "asset-manifest.json"),
  JSON.stringify({ sourceOrigin, exportedAt: new Date().toISOString(), assetCount: assetKeys.length, assets: assetKeys }, null, 2),
);
