...
export function registerStorageProxy(app: Express) {
  app.get("/manus-storage/*", async (req, res) => {
    ...
    // External hosts such as Render cannot use the platform's managed-storage
    // credentials. If a deliberate, versioned local bundle is present, serve it
    // before attempting the managed-storage fallback.
    const bundledAsset = resolveBundledStorageAssetPath(key, getBundledStorageRoot());
